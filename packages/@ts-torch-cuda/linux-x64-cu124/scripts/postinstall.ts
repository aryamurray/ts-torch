#!/usr/bin/env bun
/**
 * Postinstall script for @ts-torch-cuda/linux-x64-cu124
 * Downloads CUDA 12.4 LibTorch and builds the native library
 */
 ...
 * Skip postinstall in these cases to avoid building during local development
 */
function isDevEnvironment(): boolean {
  // Check if we're in a workspace (parent has package.json with workspaces)
  ...
  if (process.env.CI || process.env.GITHUB_ACTIONS) {
    // In CI, we want postinstall to run for testing
    return false
  }

  // Check if npm_config_global is set (global install - unlikely for this package)
...
async function postinstall(): Promise<void> {
  ...
  if (isDevEnvironment()) {
    ...
    console.log('Run "bun run scripts/postinstall.ts" manually to build.')
    return
  }
  ...
  const libPath = join(LIB_DIR, 'libts_torch.so')
