    ...
    for (const group of this.paramGroups) {
      ...
      for (const param of group.params) {
        ...
        if (
          ...
          const v2 =
            "mul" in gradSq && typeof gradSq.mul === "function"
              ? ((gradSq.mul as any)(1 - beta2) as Tensor)
              : gradSq;
        ...
        if (amsgrad) {
          ...
          if (
            ...
            state.max_exp_avg_sq = (
              state.max_exp_avg_sq as { maximum: (x: Tensor) => Tensor }
            ).maximum(state.exp_avg_sq) as Tensor;
          }
