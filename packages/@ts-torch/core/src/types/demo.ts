 ...
 */
interface LinearLayer<InFeatures extends number, OutFeatures extends number, D extends DTypeName = 'float32'> {
 ...
 */
type LinearForward<Batch extends Dim<'batch'>, InFeatures extends number, OutFeatures extends number> = MatMulShape<
  [Batch, InFeatures],
 ...
 */
interface TransformerEncoderLayer<HiddenDim extends number, FFNDim extends number, D extends DTypeName = 'float32'> {
...
interface TensorFactory {
   ...
   */
  zeros<S extends readonly number[], D extends DTypeName = 'float32'>(shape: S, dtype?: D): TensorType<S, D>

  /**
   * Ones with inferred shape
   */
  ones<S extends readonly number[], D extends DTypeName = 'float32'>(shape: S, dtype?: D): TensorType<S, D>
}
...
namespace ResNet {
  ...
  export type _Output = TensorType<BroadcastShape<[Dim<'batch'>, 64, 56, 56], [Dim<'batch'>, 64, 56, 56]>, 'float32'>
}
