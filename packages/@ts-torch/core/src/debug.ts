 ...
 */
export function validateShapesMatch(a: readonly number[], b: readonly number[], operation: string): void {
  if (!DebugMode.enabled) return

  if (a.length !== b.length) {
    throw new TypeError(`Shape mismatch in ${operation}: tensors have different ranks (${a.length} vs ${b.length})`)
  }
...
export function validateDim(shape: readonly number[], dim: number, operation: string): void {
  ...
  if (dim < -ndim || dim >= ndim) {
    throw new RangeError(`Invalid dimension ${dim} for ${operation} on tensor with ${ndim} dimensions`)
  }
}
