...
function example1_basic_tensor() {
  ...
  const handle = lib.ts_tensor_zeros(
    shapeBuffer.buffer,
    shape.length,
    dtype,
    0, // device
  ...
  const ndim = lib.ts_tensor_ndim(handle, err.buffer)
  checkError(err)
  console.log('Tensor ndim:', ndim)

  const numel = lib.ts_tensor_numel(handle, err.buffer)
  checkError(err)
  console.log('Tensor numel:', numel)

  // Cleanup
  lib.ts_tensor_delete(handle)
...
function example2_with_error() {
  ...
  const handle = withError((err) => lib.ts_tensor_ones(shape.buffer, shape.length, 0, false, err))

  console.log('Created ones tensor:', handle)

  // Get properties
  const dtype = withError((err) => lib.ts_tensor_dtype(handle, err))
  const requiresGrad = withError((err) => lib.ts_tensor_requires_grad(handle, err))
...
function example3_operations() {
  ...
  // Create two tensors
  const a = withError((err) => lib.ts_tensor_ones(shape.buffer, 2, 0, false, err))

  const b = withError((err) => lib.ts_tensor_ones(shape.buffer, 2, 0, false, err))

  console.log('Created tensors a and b')

  // Add tensors
  const sum = withError((err) => lib.ts_tensor_add(a, b, err))
  console.log('sum = a + b:', sum)

  // Multiply tensors
  const product = withError((err) => lib.ts_tensor_mul(a, b, err))
  console.log('product = a * b:', product)

  // Matrix multiplication
  const matmul = withError((err) => lib.ts_tensor_matmul(a, b, err))
...
function example4_read_data() {
  ...
  // Create tensor
  const handle = withError((err) => lib.ts_tensor_randn(shape.buffer, 2, 0, false, err))

  // Get number of elements
  const numel = withError((err) => lib.ts_tensor_numel(handle, err))
  console.log('Tensor has', numel, 'elements')

  // Allocate buffer and copy data
  const buffer = new Float32Array(Number(numel))
  withError((err) => lib.ts_tensor_copy_to_buffer(handle, buffer.buffer, BigInt(buffer.byteLength), err))

  console.log('Tensor data:', Array.from(buffer))

  // Get data pointer (alternative method)
  const dataPtr = withError((err) => lib.ts_tensor_data_ptr(handle, err))
...
function example5_scopes() {
  ...
  // Begin scope
  const scopeId = lib.ts_scope_begin()
  console.log('Scope created:', scopeId)

  try {
    // Create tensors and register with scope
    const a = withError((err) => lib.ts_tensor_ones(shape.buffer, 2, 0, false, err))
    withError((err) => lib.ts_scope_register_tensor(a, err))

    const b = withError((err) => lib.ts_tensor_ones(shape.buffer, 2, 0, false, err))
    withError((err) => lib.ts_scope_register_tensor(b, err))

    const c = withError((err) => lib.ts_tensor_add(a, b, err))
    withError((err) => lib.ts_scope_register_tensor(c, err))

    console.log('Created and registered 3 tensors')

    // Escape tensor c from automatic cleanup
    withError((err) => lib.ts_scope_escape_tensor(c, err))
    console.log('Escaped tensor c from scope')

    // End scope - only a and b will be deleted
    withError((err) => lib.ts_scope_end(scopeId, err))
    console.log('Scope ended - a and b automatically deleted')

    // Manually delete escaped tensor
    lib.ts_tensor_delete(c)
    console.log('Manually deleted tensor c')
  } catch (err) {
    // Cleanup on error
    withError((e) => lib.ts_scope_end(scopeId, e))
    throw err
  }
}
...
function example6_activations() {
  ...
  const x = withError((err) => lib.ts_tensor_randn(shape.buffer, 2, 0, false, err))

  // Apply various activations
  const relu = withError((err) => lib.ts_tensor_relu(x, err))
  console.log('ReLU applied:', relu)

  const sigmoid = withError((err) => lib.ts_tensor_sigmoid(x, err))
  console.log('Sigmoid applied:', sigmoid)

  const tanh = withError((err) => lib.ts_tensor_tanh(x, err))
  console.log('Tanh applied:', tanh)

  const softmax = withError((err) => lib.ts_tensor_softmax(x, -1, err))
  console.log('Softmax applied:', softmax)

  // Cleanup
  ;[x, relu, sigmoid, tanh, softmax].forEach((h) => {
    lib.ts_tensor_delete(h)
  })
}
...
function example7_autograd() {
  ...
  const x = withError(
    (err) => lib.ts_tensor_ones(shape.buffer, 2, 0, true, err), // requires_grad = true
  )

  const y = withError((err) => lib.ts_tensor_ones(shape.buffer, 2, 0, true, err))

  // Perform operations
  const z = withError((err) => lib.ts_tensor_mul(x, y, err))
  const loss = withError((err) => lib.ts_tensor_sum(z, -1, false, err))

  console.log('Created computation graph: loss = sum(x * y)')

  // Backward pass
  withError((err) => lib.ts_tensor_backward(loss, err))
  console.log('Backward pass completed')

  // Get gradients
  const xGrad = withError((err) => lib.ts_tensor_grad(x, err))
  const yGrad = withError((err) => lib.ts_tensor_grad(y, err))

  console.log('x.grad:', xGrad)
  console.log('y.grad:', yGrad)

  // Zero gradients
  withError((err) => lib.ts_tensor_zero_grad(x, err))
  withError((err) => lib.ts_tensor_zero_grad(y, err))
  console.log('Gradients zeroed')

  // Cleanup
  ;[x, y, z, loss].forEach((h) => lib.ts_tensor_delete(h))

  if (xGrad) lib.ts_tensor_delete(xGrad)
  if (yGrad) lib.ts_tensor_delete(yGrad)
}
...
function example8_cuda() {
  ...
  const cudaAvailable = lib.ts_cuda_is_available()
  console.log('CUDA available:', cudaAvailable)

  if (cudaAvailable) {
    const deviceCount = lib.ts_cuda_device_count()
    ...
    if (deviceCount > 0) {
      ...
      const cpuTensor = withError((err) => lib.ts_tensor_randn(shape.buffer, 2, 0, false, err))

      console.log('Created CPU tensor:', cpuTensor)

      // Move to GPU
      const gpuTensor = withError((err) =>
        lib.ts_tensor_to_device(
          cpuTensor,
      ...
      const result = withError((err) => lib.ts_tensor_relu(gpuTensor, err))
...
function example9_version() {
  ...
  // Get version string
  const versionPtr = lib.ts_version()
