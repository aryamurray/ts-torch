...
function example2_with_error() {
  ...
  const handle = withError((err) => lib.symbols.ts_tensor_ones(ptr(shape), shape.length, 0, false, err))
...
function example4_read_data() {
  ...
  withError((err) => lib.symbols.ts_tensor_copy_to_buffer(handle, ptr(buffer), BigInt(buffer.byteLength), err))
...
function example8_cuda() {
  ...
  if (cudaAvailable) {
    ...
    if (deviceCount > 0) {
      ...
      const cpuTensor = withError((err) => lib.symbols.ts_tensor_randn(ptr(shape), 2, 0, false, err))
